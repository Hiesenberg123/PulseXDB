...
uimedia <- function(id, label = "Counter") {
  ...
  semanticPage(
    ...
    title = h2(class = "ui header", icon("newspaper"), div(class = "content", "Media Insight")), 
    includeCSS("main.css"),
...
mediaServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$artcnt1 <- renderText({
        str1<- paste("Total Articles: ", dim(Final())[1], " ")
        str2<- paste("Hindi Articles: ", dim(Final()[Final()$lang== "hi",])[1], " ")
        str3<- paste("English Articles: ", dim(Final()[Final()$lang== "en",])[1], " ")
          
        paste(str1, str2, str3, sep = "\n")
      })
    
    output$src1 <- renderText({
      str1<- paste("Unique Sources: ", dim(count(Final(), Source))[1], " ")
      str2<- paste("Hindi Sources: ", dim(Final()[Final()$lang== "hi",])[1], " ")
      str3<- paste("English Sources: ", dim(Final()[Final()$lang== "en",])[1], " ")
    ...
    output$choice1<- renderUI({
      ...
      selectInput(
        ...
        choices = get_choice()$sub[get_choice()$topic== input$Topic1],
        selected = get_choice()$sub[get_choice()$topic== input$Topic1][1],
        width = 200
      )
    })
    ...
    output$senti_bd1<- renderPlot({
     ...
     output$wd_freq12<- renderPlot({
       wd_freq22(Final(), c("mafia", "growth", "covid"))
     })
